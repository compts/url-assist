...
const parseStringConvert=function (key, value, type, config, reference) {
    ...
    if (_stk.indexOf(type, [
        ...
        _stk.each(value, function (vl, ky) {
            ...
            const keyVal = _stk.indexOf(type, [
                "number",
                "array"
            ]) >=zero
                ?config.arrayFormat
...
const parseObjectSchema = function (referenceValue, defaultConfig, keyOnly, keyList, getValueOnly, reFlistKey, isParent) {
    ...
    const keyRefArray = _stk.toArray(keyList);

    const keyFlatten = _stk.flatten([
        keyOnly,
        keyRefArray
    ]);
    ...
    if (_stk.indexOfExist(keyOnly, reFlistKey)) {
        ...
        if (_stk.indexOfExist(_stk.getTypeof(referenceValue[keyOnly]), [
            ...
            const isrefExist = _stk.someValid(_stk.map(function (params) {
...
function basePattern (pattern) {
    ...
    if (_stk.getTypeof(pattern) ==="json") {
        ...
        const patternRegexp = _stk.has(pattern, "regexp")
            ? pattern.regexp
            : "--";
        const listArgument = _stk.has(pattern, "regexp")
            ? pattern.arguments
            : "--";
    ...
    if (_stk.getTypeof(pattern) ==="regexp") {
        ...
        const listArgument = _stk.map(function (value) {
...
const isUrlValidFormatVerifier=function (domain, config) {
    ...
    if (httpRegExp.test(domain)) {
        ...
        const filterEmpty = _stk.filter(function (valS) {
        ...
        if (_stk.count(cleanUrlSplit) >= two) {
            ...
            const tldName = _stk.last(cleanUrlSplit);
            const getTLD = _stk.count(_stk.first(tldName.split("/")).split(""));
...
function joinUrlPath (...ags) {
    ...
    const cleanReplacePath = _stk.reduce(function (grand, value) {
...
function isWSProtocolValid (host, config) {
    ...
    const varConfig = _stk.varExtend({
        "isSecure": true,
        "isValidFormat": true
    }, config);
